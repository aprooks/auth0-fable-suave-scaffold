...
let result = handler.ValidateToken(token,par,jwtToken)

let claims token = 
    let jwtToken = JwtSecurityToken() :> Microsoft.IdentityModel.Tokens.SecurityToken |> ref
    
    handler.ValidateToken(token,par,jwtToken)
